...
def create_prompt(conversation: List[Message], language: str, max_output_tokens: int) -> dict:
    ...
    """
    system_prompts = {
        'pt': """Analise a seguinte conversa do WhatsApp e liste os 5 à 9 principais temas discutidos.
        ...
        'en': """Analyze the following WhatsApp conversation and list 5 to 9 main themes discussed.
...
def format_conversation_within_token_limit(messages: List[str], system_prompt: str, max_output_tokens: int) -> str:
    ...
    max_conversation_tokens = max_tokens - base_tokens - max_output_tokens - 100  # Leave some buffer
    
    conversation_text = "\n".join(messages)
    while count_tokens(conversation_text) > (max_conversation_tokens - max_output_tokens) \
        and messages:
...
def parse_themes_response(response_json: dict) -> Dict[str, str]:
    ...
    try:
        ...
        for theme_item in themes_array:
            ...
            # Get examples (support both English and Portuguese keys, and both single and array formats)
            examples = theme_item.get('examples', theme_item.get('exemplos', theme_item.get('example', [])))
...
def sample_conversation(conversation: List[Message], sample_size: int = 50, num_samples: int = 20) -> List[Message]:
    """
    ...
    selected_starts = random.sample(
        valid_start_indices, 
        min(num_samples, len(valid_start_indices))
    )
