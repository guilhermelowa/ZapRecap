...
def get_suggestions_stats():
    ...
    pending = session.query(func.count(Suggestion.id)).filter(Suggestion.status == "pending").scalar()
    by_date = session.query(
        func.date(Suggestion.timestamp),
        func.count(Suggestion.id)
    ).group_by(func.date(Suggestion.timestamp)).all()
